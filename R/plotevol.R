    ...
    STAMP = NULL, STYLE = "fft")
{
    ...
    trace = RESCALE(a, perc, 1, min(a, na.rm = TRUE), max(a,
        na.rm = TRUE))
    lines(tim, trace)
    ...
    if (ylog == TRUE) {
        ...
        raxspec = RESCALE(axspec, 0, perc, min(log10(y[yflag]),
        ...
        raxspec = RESCALE(axspec, 0, perc, min(y[yflag], na.rm = TRUE),
    ...
    if (axtrace[1] < axtrace[2]) {
        raxtrace = RESCALE(axtrace, perc, 1, axtrace[1], axtrace[2])
